...
- DexPaprika (публичная бета, без ключа) — **основной источник** цены, market cap (FDV) и ликвидности для Robinhood Chain:
  ...
  - Почему не только Blockscout? Blockscout на этой сети **не отдаёт цену** (`exchange_rate`/`circulating_market_cap` = `null` даже для реально торгуемых ERC-20), поэтому посчитать памп по нему нельзя. Цена есть только на DEX. Blockscout используется для типа токена, holders и supply.

## Фильтрация NFT и обработка 404

- Событие `Transfer(address,address,uint256)` одинаково у ERC-20 **и** ERC-721, поэтому в RPC logs попадают и NFT (например «Uniswap V3 Positions»). Такие адреса потом дают `404` на каждом запросе цены.
- Решение: на этапе discovery для каждого адреса берётся `type` из Blockscout, и если он явно **не** `ERC-20` (ERC-721/1155) — токен пропускается. Ещё не проиндексированные адреса не отбрасываются, чтобы не терять свежие токены.
- `404`/`400` от DEX и Relay теперь **не ретраятся** и логируются на уровне `debug`: это просто означает «токен ещё не торгуется / не в базе агрегатора», а не ошибку. Раньше каждый такой ответ повторялся 5 раз с `WARNING` и засорял лог.
